...
| <a name="input_ecs_efs_volumes"></a> [ecs\_efs\_volumes](#input\_ecs\_efs\_volumes) | (Optional) A set of volume blocks that containers in your task may use | <pre>list(object({<br>    name = string<br>    authorization_config = object({<br>      access_point_id = string<br>    })<br>    efs_volume_configuration = object({<br>      file_system_id = string<br>      root_directory = string<br>    })<br>  }))</pre> | `[]` | no |
